#!/usr/bin/env node
/**
 * Exact local-only T3 experiment-rework worker.
 *
...
let worktree = '';
let baselineGateWorktree = '';
let candidateGateWorktree = '';
try {
  ...
  // From this point onward the authoring checkout is evidence-only. The model
  // may have left a delayed/background writer behind; gates therefore run in
  // independent, non-shared clones created from the exact baseline and the
  // already captured patch bytes. A second sealed clone replays the reviewed
  // parent only to preserve the established "new diagnostics" comparison.
  baselineGateWorktree = mkdtempSync(path.join(tmpdir(), 'swipe-wild-baseline-gate-'));
  rmSync(baselineGateWorktree, { recursive: true, force: true });
  const baselineGate = await createSealedGateClone({
    sourceRepo: playablesRoot,
    destination: baselineGateWorktree,
    baseCommit,
    sourcePath: baseline.sourcePath,
    sourceTree: workerInput.baseline.sourceTree,
    patchBytes: parent.patchBytes,
    dependencies,
    runGit: runTrustedGit,
    normalizeIndexFlags,
  });
  status('typecheck-baseline', 'Recording reviewed-parent diagnostics in a sealed clone');
  const baselineDiagnostics = new Set((await collectTypeDiagnostics(
    baselineGate.worktree,
    baselineGate.integrity,
  )).map(normalizeDiagnostic));

  candidateGateWorktree = mkdtempSync(path.join(tmpdir(), 'swipe-wild-candidate-gate-'));
  rmSync(candidateGateWorktree, { recursive: true, force: true });
  const candidateGate = await createSealedGateClone({
    sourceRepo: playablesRoot,
    destination: candidateGateWorktree,
    baseCommit,
    sourcePath: baseline.sourcePath,
    sourceTree: workerInput.baseline.sourceTree,
    patchBytes: Buffer.from(validated.patch, 'utf8'),
    dependencies,
    runGit: runTrustedGit,
    normalizeIndexFlags,
  });
  const sealedValidated = await validateDiff(
    candidateGate.worktree,
    baseCommit,
    candidateGate.integrity,
  );
  assertCapturedPatchReplay(candidateGate.capturedPatch, sealedValidated.patch);
  if (JSON.stringify(sealedValidated.files) !== JSON.stringify(validated.files)) {
    const error = new Error('sealed clone changed the captured patch file identity');
    error.code = 'sealed_gate_clone_invalid';
    throw error;
  }
  await typecheck(
    candidateGate.worktree,
    sealedValidated.files,
    baselineDiagnostics,
    1,
    candidateGate.integrity,
  );
  ...
  if (baselineGateWorktree) rmSync(baselineGateWorktree, { recursive: true, force: true });
  if (candidateGateWorktree) rmSync(candidateGateWorktree, { recursive: true, force: true });
}
